...
class AdminNewsCategoriesController extends Controller
{
	...
	public function deletenewscategory(Request $request){
		...
		$newses = DB::table('news_to_news_categories')->where('categories_id',$request->id)->get();
		
		foreach($newses as $news){
			DB::table('news')->where('news_id',$news->news_id)->delete();
			DB::table('news_description')->where('news_id',$news->news_id)->delete();
		}
