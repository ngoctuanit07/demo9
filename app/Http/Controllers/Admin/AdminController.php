...
class AdminController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        foreach ($orders as $orders_data) {
            ...
            foreach ($orders_products as $orders_product) {
                ...
                $single_purchased_price = DB::table('inventory')->where('products_id',
                    $orders_product->products_id)->sum('purchase_price');
                $single_stock           = DB::table('inventory')->where('products_id',
                    $orders_product->products_id)->where('stock_type', 'in')->sum('stock');
                if ($single_stock > 0) {
            ...
            $orders_status_history = DB::table('orders_status_history')
                ->LeftJoin('orders_status', 'orders_status.orders_status_id', '=',
                    'orders_status_history.orders_status_id')
                ->select('orders_status.orders_status_name', 'orders_status.orders_status_id')
                ->where('orders_id', '=', $orders_data->orders_id)->orderby('orders_status_history.date_added',
                    'DESC')->limit(1)->get();
        ...
        foreach ($products as $products_data) {
            ...
            if (count($currentStocks) > 0) {
                ...
                if ($products_data->products_type != 1) {
                    $c_stock_in  = DB::table('inventory')->where('products_id',
                        $products_data->products_id)->where('stock_type', 'in')->sum('stock');
                    $c_stock_out = DB::table('inventory')->where('products_id',
                        $products_data->products_id)->where('stock_type', 'out')->sum('stock');
    ...
    public function admininfo()
    {
        ...
        return view("admin.login", $administor);
    }
            ...
            if (auth()->guard('admin')->attempt($adminInfo)) {
                ...
                if (!empty(auth()->guard('admin')->user()->adminType)) {
                    if (auth()->guard('admin')->user()->adminType != '1') {
                        $roles = DB::table('manage_role')->where('admin_type_id',
                            auth()->guard('admin')->user()->adminType)->get();
                ...
                //admin category role
                if (auth()->guard('admin')->user()->adminType != '1') {
                    $categories_role = DB::table('categories_role')->where('admin_id',
                        auth()->guard('admin')->user()->myid)->get();
    ...
    public function adminProfile(Request $request)
    {
        ...
        if (session('profile_view') == 0) {
            ...
            $zones     = DB::table('zones')->where('zone_country_id', '=',
                auth()->guard('admin')->user()->country)->get();
    ...
    public function updateProfile(Request $request)
    {
        ...
        if (session('profile_update') == 0) {
            ...
            $orders_status = DB::table('administrators')->where('myid', '=',
                auth()->guard('admin')->user()->myid)->update([
    ...
    public function addadmins(Request $request)
    {
        ...
        $adminTypes           = DB::table('admin_types')->where('isActive', 1)->where('admin_type_id', '!=',
            '1')->get();
    ...
    public function updateadmin(Request $request)
    {
        if (session('manage_admins_update') == 0) {
            ...
            //check email already exists
            $existEmail = DB::table('administrators')->where([
                ['email', '=', $request->email],
                ['myid', '!=', $myid],
            ])->get();
            if (count($existEmail) > 0) {
    ...
    public function categoriesroles(Request $request)
    {
        if (session('manage_admins_role') == 0) {
            ...
            $categories_role = DB::table('administrators')->join('categories_role', 'categories_role.admin_id', '=',
                'administrators.myid')->where('administrators.adminType', '!=', '1')->get();
            ...
            foreach ($categories_role as $categories) {
                ...
                $categories_descrtiption     = DB::table('categories_description')->whereIn('categories_id',
                    $cat_array)->where('language_id', $language_id)->get();
