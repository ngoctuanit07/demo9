...
class AdminAlertController extends Controller
{
	...
	public function orderStatusChange($data){
		...
		//status change email 
		if($alertSetting[0]->order_status_email==1 and count($temp['devices'])>0 and !empty($temp['devices'][0]->email)){	
		
			Mail::send('/mail/orderStatus', ['data' => $temp], function($m) use ($temp){
				$m->to($temp['devices'][0]->email)->subject(Lang::get("labels.OrderStatusEmailTitle"))->getSwiftMessage()
				->getHeaders()
	...
	public function newProductNotification($products_id){
		...
		if($alertSetting[0]->new_product_notification==1){
			...
			//image			
			$websiteURL =  "http://" . $_SERVER['SERVER_NAME'] .'/demos/website/'. $product[0]->products_image;	
	...
	public function newsNotification($news_id){
		...
		if($alertSetting[0]->news_notification==1){
			...
			//image			
			$websiteURL =  "http://" . $_SERVER['SERVER_NAME'] .'/demos/website/'. $news[0]->news_image;	
