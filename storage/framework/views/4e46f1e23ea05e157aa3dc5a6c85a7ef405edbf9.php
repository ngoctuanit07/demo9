...
$(document).on('click', '.currentstock', function(e){
	...
	$.ajax({
		url: '<?php echo e(URL::to("admin/currentstock")); ?>',
		type: "POST",
		data: formData,
		dataType: "json",
...
$(document).on('click', '#addAttribute', function(e){
	...
	$.ajax({
		...
		success: function (res) {
			if(res == 'empty'){
				$('.addError span').html('<?php echo e(trans("labels.AddOptionEmptyErrorText")); ?>');
				$('.addError').show();
			}else if(res == 'already'){
				$('.addError span').html('<?php echo e(trans("labels.AddOptionErrorText")); ?>');
				$('.addError').show();
			}else{
				$('.addError').hide();
				$('#addAttributeModal').modal('hide');
...
$(document).on('click', '#addDefaultAttribute', function(e){
	...
	$.ajax({
		...
		success: function (res) {
			if(res == 'empty'){
				$('.addDefaultError span').html('<?php echo e(trans("labels.AddOptionEmptyErrorText")); ?>');
				$('.addDefaultError').show();
			}else if(res == 'already'){
				$('.addDefaultError span').html('<?php echo e(trans("labels.AddOptionErrorText")); ?>');
				$('.addDefaultError').show();
			}else{
				$('.addError').hide();
...
function cancelOrder() {
	...
	if(status_id==3){
		if (confirm("<?php echo app('translator')->getFromJson('labels.Are you sure you want to cancel this order?'); ?>")) {
			return true;
		} else {
			return false;
		}
