...
jQuery( document ).ready( function () {
	...
	jQuery.ajax({
		url: '<?php echo e(URL::to("/cartButton")); ?>',
		type: "GET",		
		success: function (res) {
			jQuery('.head-cart-content').html(res);	
		},
...
jQuery(document).ready(function(e) {
   ...
   <?php if(!empty($result['flash_sale']['success']) and $result['flash_sale']['success']==1): ?>
	   ...
	   <?php if( $products->server_time >= $products->flash_start_date): ?>
		...
		var distance_<?php echo e($products->products_id); ?> = <?php echo e($products->flash_expires_date); ?> - <?php echo e($products->server_time); ?>;
				
		// Update the count down every 1 second
		var x_<?php echo e($products->products_id); ?> = setInterval(function() {
		    distance_<?php echo e($products->products_id); ?> = distance_<?php echo e($products->products_id); ?> - 1;
		   
		
		  // Time calculations for days, hours, minutes and seconds
			  var days_<?php echo e($products->products_id); ?> = Math.floor(distance_<?php echo e($products->products_id); ?> / ( 60 * 60 * 24));
			  var hours_<?php echo e($products->products_id); ?> = Math.floor((distance_<?php echo e($products->products_id); ?> % ( 60 * 60 * 24)) / ( 60 * 60));
			  var minutes_<?php echo e($products->products_id); ?> = Math.floor((distance_<?php echo e($products->products_id); ?> % ( 60 * 60)) / ( 60));
			  var seconds_<?php echo e($products->products_id); ?> = Math.floor(distance_<?php echo e($products->products_id); ?> % (60));
			  ...
			  document.getElementById('product_div_<?php echo e($products->products_id); ?>').style.display = 'block'; 
   ...
   <?php endif; ?>	
	...
	<?php if(!empty($result['detail']['product_data'][0]->flash_start_date)): ?>
		<?php if( $result['detail']['product_data'][0]->server_time >= $result['detail']['product_data'][0]->flash_start_date): ?>
			var distance = <?php echo e($result['detail']['product_data'][0]->flash_expires_date); ?> - <?php echo e($result['detail']['product_data'][0]->server_time); ?>;
...
function cancelOrder() {
	if (confirm("<?php echo app('translator')->getFromJson('website.Are you sure you want to cancel this order?'); ?>")) {
		return true;
	} else {
		return false;
	}
...
function returnOrder() {
	if (confirm("<?php echo app('translator')->getFromJson('website.Are you sure you want to return this order?'); ?>")) {
		return true;
	} else {
		return false;
	}
...
jQuery(document).on('click', '#subscribe', function(e){
	...
	if(this.email != '' && validEmail.test(email)) {
		...
		jQuery('.error-subscribte').show();
		jQuery('.error-subscribte').html("<?php echo app('translator')->getFromJson('website.Please enter your email address'); ?>");	
